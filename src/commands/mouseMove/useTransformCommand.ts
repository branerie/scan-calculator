...
const useTransformCommand = () => {
  ...
  const handleTransformCmd = useCallback(
    ({ mouseX, mouseY }: MousePosition) => {
      ...
      if (tool.name === 'move') {
        ...
        for (const editedElement of newCurrentlyEditedElements.values()) {
          const newEditedElement = ElementManipulator.copyElement(editedElement, {
            keepIds: true,
            assignId: false,
          }) as ElementWithId
      ...
      if (tool.name === 'rotate') {
        ...
        for (const editedElement of newCurrentlyEditedElements.values()) {
          ...
          for (const selectionPoint of selectionPoints) {
            ...
            newEditedElement.setPointById(selectionPoint.pointId, newPointPosition.x, newPointPosition.y)
          }
      ...
      if (tool.name === 'scale') {
        ...
        for (const editedElement of newCurrentlyEditedElements.values()) {
          ...
          for (const selectionPoint of selectionPoints) {
            const lineToPoint = createLine(initialClick.x, initialClick.y, selectionPoint.x, selectionPoint.y)

            lineToPoint.setLength(lineToPoint.length * scalingFactor, false)
            newEditedElement.setPointById(selectionPoint.pointId, lineToPoint.pointB.x, lineToPoint.pointB.y)
          }
    ...
    [tool, toolClicks, currentlyEditedElements, changeEditingElements, getElementById, currentScale]
  )
