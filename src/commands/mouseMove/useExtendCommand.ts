...
const getExtensionDifference = <T extends Element>(
  ...
  if (original instanceof Line) {
    ...
    let extensionElements = getExtensionDifference(original.elements[0], (result as FullyDefinedPolyline).elements[0])!
...
const useExtendCommand = () => {
  ...
  const handleExtendCmd = useCallback((mousePosition: MousePosition) => {
    if (!tool.isStarted) {
      return
    }
    ...
    let elementsToExtend = lastClick
      ? getElementsInContainer(lastClick, mousePoint, { shouldSkipPartial: false, returnGroup: 0 }) as ElementWithId[] | null
      : getElementsContainingPoint(mouseX, mouseY, { maxPointsDiff: selectDelta, returnGroup: 0 }) as ElementWithId[] | null
    ...
    const retrieveElementCommandResult = (
      ...
      if (newStartPos || newEndPos) {
        let editedElement = ElementManipulator.copyElement(elementToExtend, { assignId: true }) as FullyDefinedElement
    ...
    for (const elementToExtend of elementsToExtend) {
      ...
      if (lastClick) {
        const selectRect = createElementFromName('rectangle', createPoint(lastClick.x, lastClick.y, { assignId: false }))
    ...
    for (const filteredElement of filteredElementsToExtend) {
      ...
      if (result) {
        ...
        commandResult.set(elementId, {
          ...result,
          diffElements: getExtensionDifference(
