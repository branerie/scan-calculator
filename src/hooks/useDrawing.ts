...
const useDrawing = () => {
  ...
  const toolKeys = toolsStore(state => state.toolKeys)
  const currentTranslate = toolsStore(state => state.currentTranslate)
  const currentScale = toolsStore(state => state.currentScale)

  const { canvasContext } = useAppContext()

  const elementsContainer = useElementContainerContext()
  const container = (elementsContainer as HashGridElementContainer)
  // @ts-ignore
  ...
  const resetCanvas = useCallback(() => {
    ...
    const drawElement = useCallback((element: FullyDefinedElement, options: DrawOptions = {}) => {
    ...
    const drawSelectedElement = useCallback((element: FullyDefinedElement, options: DrawOptions = {}) => {
      drawElement(element, {
        ...options,
    ...
    const drawSelectionPoints = useCallback((selectionPoints: SelectionPoint[]) => {
      if (!canvasContext) {
        return
      }
