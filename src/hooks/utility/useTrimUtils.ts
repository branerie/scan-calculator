...
const useTrimUtils = () => {
  ...
  const getElementsNearElement = elementsStore(state => state.getElementsNearElement)
  ...
  * }
  */
  const getSingleElementTrimResults = useCallback((elementsToTrim: ElementWithId[], pointsOfSelection: Point[]) => {
    const commandResult: Map<string, { replacingElements: ElementWithId[], removedSections: FullyDefinedElement[] }> = new Map()
    // const polylines: Record<string, Record<string, Point[]>> = {}
    const polylineIds: Set<string> = new Set()
    for (const elementToTrim of elementsToTrim) {
      if (elementToTrim.baseType === 'polyline') {
        // polylines[elementToTrim.id] = {}
        polylineIds.add(elementToTrim.id)
        continue
      }
  ...
  const getPolylineTrimResults = useCallback(
    // (polylines: Record<string, Record<string, Point[]>>, pointsOfSelection: Point[]) => {
    (polylineIds: Set<string>, pointsOfSelection: Point[]) => {
      ...
      const filterTrimPoints = (
        ...
        for (const suTrimPoint of currentSubElementTrimPoints) {
          ...
          const selectedSuIdsWithPoint = Array.from(
            elementsByTrimPoint[pointKey]
          ).filter(subElementId => selectedSubElementIds.has(subElementId))
      ...
      const commandResult: Map<string, { replacingElements: ElementWithId[], removedSections: Element[] }> = new Map()
        ...
        if (resultElements) {
          for (
            ...
            if (replacingPolyline.elements.length === 1) {
              resultElements.remaining[replacingIndex] = replacingPolyline.elements[0] as Ensure<FullyDefinedElement, 'id'>
