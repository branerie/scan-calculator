  ...
  > = ((
    ...
    return {
      ...
      editElements() {
        ...
        const elementPointsBeforeEdit = new Map<string, SelectionPoint[]>()
        ...
        for (const cee of editedElementsValuesArr) {
          ...
          elementPointsBeforeEdit.set(
            cee.id, 
            elementBeforeEdit.getSelectionPoints()
          )
        }
        ...
        if (!!selectedPoints) {
          ...
          pointsToReplace = editedElementsValuesArr.reduce((acc, cee) => (
            acc.concat(cee.getSelectionPoints())
          ), new Array<SelectionPoint>())
        }

        for (const pointToReplace of pointsToReplace) {
          const elementOfPoint = currentlyEditedElements.get(pointToReplace.elementId)
          if (!elementOfPoint) {
            throw new Error(
          ...
          const selectionPointsBeforeEdit = elementPointsBeforeEdit.get(elementOfPoint.id)
