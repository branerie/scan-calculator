...
export default function useElementsSlice() {
  ...
  const createElementsSlice: StateCreator<
    ElementsState,
    [["zustand/immer", never]],
    [],
    ElementsSlice
    ...
    return {
      ...
      getElementsInContainer(
        ...
        for (const elementId of elementIds) {
          ...
          const intersections = ElementIntersector.getIntersections(element, container as FullyDefinedRectangle)
          if (intersections) {
            const elementsToAdd = getElementsFromReturnGroupOptions(element, returnGroup)
            elementsInContainer = elementsInContainer.concat(elementsToAdd)
          }
      ...
      completeEditingElements() {
        let editedElements: ElementWithId[] = []
        set((state) => {
          ...
          editedElements = Array.from(
            (currentlyEditedElements! as Map<string, ElementWithId>).values()
          )
