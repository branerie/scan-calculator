    ...
    immer<ToolsState>((set, get) => ({
      ...
      zoomView(centerX, centerY, isZoomOut) {
        ...
        const newScale = Math.min(
          Math.max(currentScale * scaleStep, VIEW_ZOOM_MIN_SCALE), 
          VIEW_ZOOM_MAX_SCALE
        )
      ...
      addToolClick(clickedPoint, isReferenceClick = false) {
        set((state) => {
          if (state.toolClicks) {
      ...
      getLastReferenceClick() {
        ...
        if (
          !toolClicks || 
          (!tool.refClickIndex && tool.refClickIndex !== 0)
        ) {
          return null
        }
      ...
      getRealMouseCoordinates(clientX, clientY) {
        ...
        return [
          (clientX - translateX) / currentScale,
          (clientY - translateY) / currentScale
        ]
      },
      startUsingTool() {
        set((state) => {
